...
class Buffer {
  ...
  void scrollDown(int lines) {
    // Collect items that will be kept (shifted down), then reassign.
    // Direct in-place shifting via operator[]= corrupts the circular buffer:
    // _adoptChild detaches the source item's old slot on the next iteration,
    // leaving a detached BufferLine at the target slot.
    ...
    for (var i = top; i <= bottom - lines; i++) {
      kept.add(this.lines[i]);
    }
    for (var i = bottom; i >= top; i--) {
      if (i >= top + lines) {
        this.lines[i] = kept[i - top - lines];
      } else {
  ...
  void scrollUp(int lines) {
    // Collect items that will be kept (shifted up), then reassign.
    // See scrollDown comment for rationale.
    final top = absoluteMarginTop;
    final bottom = absoluteMarginBottom;
    final kept = <BufferLine>[];
    for (var i = top + lines; i <= bottom; i++) {
      kept.add(this.lines[i]);
    }
    for (var i = top; i <= bottom; i++) {
      if (i <= bottom - lines) {
        this.lines[i] = kept[i - top];
      } else {
        this.lines[i] = _newEmptyLine();
      }
    }
